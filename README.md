...
13 тестов в [`tests/integration/api.test.ts`](tests/integration/api.test.ts) — реальный Postgres через [Testcontainers](https://node.testcontainers.org/), `prisma migrate deploy` + seed, обращения через `app.inject(...)` (без сети):
...
| **Prisma 6 (а не v7)** | Pinned `6.16.x` LTS — стабильная ветка с зрелым tooling. Prisma 7 переносит `datasource.url` в `prisma.config.ts` и требует driver adapter pattern (`@prisma/adapter-pg`) — миграция не приносит бизнес-выгоды для этого сервиса. **Note:** VS Code-расширение Prisma последних версий валидирует схему по правилам v7 и подсвечивает строку `url = env("DATABASE_URL")` как ошибку — это false positive для нашего CLI v6, где эта строка обязательна. |
